  ...
  if (!isLoggedIn) {
    ...
    <div style={styles.JoinGame}>
      ...
      <div style={styles.flexStyle}>
        <div >
          {rooms.map(Game => (
              <div key={Game.game.id} style={{display: 'flex', justifyContent : 'space-between', border : "1px solid black"}}>
                <div>id: {Game.game.id} </div>
                <div>Number of players: {Game.game.player.length} </div>
                
                <div>started: {Game.game.start == true ? 'yes' : 'no'} </div>
