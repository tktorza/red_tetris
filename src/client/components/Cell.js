...
const Cell = (props) => {
   ...
   if (currentPiece){
       ...
       for (let i = 0; i < endLine.length; i++){
        ...
        if(end === true){
            return(<div key={column.id} style={{width: '2em', height: '2em', backgroundColor: 'red'}} className={tab.id}/>)
        }else if (color === true){
            return(<div key={column.id} style={{width: '2em', height: '2em', backgroundColor: backgroundPiece}} className={tab.id}/>)
        
        }else{
            return(<div key={column.id} style={{width: '2em', height: '2em', backgroundColor: 'white'}} className={tab.id}/>)
